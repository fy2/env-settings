...
function! s:RunGrepRecursive(cmd_name, grep_cmd, action, ...)
    ...
    if v:version >= 700
" Feyruz        let startdir = input("Start searching from directory: ", cwd, "dir")
        let startdir = cwd
    else
" Feyruz       let startdir = input("Start searching from directory: ", cwd)
        let startdir = cwd
    endif
    if startdir == ""
        return
    ...
    if filepattern == ""
" Feyruz        let filepattern = input("Search in files matching pattern: ", 
" Feyruz                                         \ g:Grep_Default_Filelist)
